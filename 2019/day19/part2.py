...
def solve(inp):
  ...
  while True:
    ...
    for i in range(100):
      nx = x + i
      if (nx, y) not in hist:
        hist[(nx, y)] = run_code(opcodes, nx, y)
    ...
    for j in range(100):
      ny = y + j
      if (x, ny) not in hist:
        hist[(x, ny)] = run_code(opcodes, x, ny)

...
class Cross:
    ...
    def e0_dist(self):
        global roots
        res = abs(self.x - self.e0.node1.x) + abs(self.y - self.e0.node1.y)
        return self.e_backtrack(res, self.e0.node1)

    @property
    def e1_dist(self):
        global roots
        res = abs(self.x - self.e1.node1.x) + abs(self.y - self.e1.node1.y)
        return self.e_backtrack(res, self.e1.node1)

    def e_backtrack(self, res, node_pointer):
        while True:
            if node_pointer.edges[0].node1 == node_pointer:
                break
            res += node_pointer.edges[0].length
            node_pointer = node_pointer.edges[0].node1
...
class Edge:
    ...
    def add_if_cross(self, e2):
        ...
        if (self.is_horizontal and not e2.is_horizontal) and \
            ...
            crosses.append(Cross(e2.node1.x, self.node1.y, self, e2))
            ...
            crosses.append(Cross(self.node1.x, e2.node1.y, self, e2))
...
def solve(inp):
    ...
    for cross in crosses:
        print('x: {}\ty: {}\te0_dist: {}\te1_dist: {}\ttot_dist: {}'.format(cross.x, cross.y,
                                                                            cross.e0_dist, cross.e1_dist,
                                                                            cross.e0_dist + cross.e1_dist))

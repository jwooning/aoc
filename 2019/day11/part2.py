...
def solve(inp):
    ...
    xmin = min([x for x, _ in hull.keys()])
    xmax = max([x for x, _ in hull.keys()])
    ymin = min([y for _, y in hull.keys()])
    ymax = max([y for _, y in hull.keys()])
    dx = xmax - xmin
    dy = ymax - ymin

    license = np.zeros(shape=(dx + 1, dy + 1), dtype=np.int)
    for pos, color in hull.items():
        license[pos[0] - xmin, pos[1] - ymin] = color
    plt.imshow(license)
    plt.show()

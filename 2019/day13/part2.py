...
def solve(inp):
    ...
    while True:
        ...
        old_ball = ball
        ball = np.where(screen == 4)[1][0]
        paddle = np.where(screen == 3)[1][0]
        ball_dir = 1 if ball > old_ball else -1

        if np.where(screen == 4)[0][0] + 1 == np.where(screen == 3)[0][0] and paddle == ball:
            icem.inputs.append(0)
        if paddle < ball + ball_dir:
            icem.inputs.append(1)
        elif paddle > ball + ball_dir:
            icem.inputs.append(-1)
        else:
            icem.inputs.append(0)

        stdscr.addstr(screen.shape[0] + 1, 0, 'BALL: {}'.format(np.where(screen == 4)[0][0]))
        stdscr.addstr(screen.shape[0] + 2, 0, 'PADDLE: {}'.format(np.where(screen == 3)[0][0]))
        stdscr.addstr(screen.shape[0] + 3, 0, 'BALL_DIR: {}'.format(ball_dir))
        stdscr.addstr(screen.shape[0] + 4, 0, 'INPUTS: {}'.format(icem.inputs))
